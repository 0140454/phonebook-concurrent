...
int main(void)
{
    ...
    fp = fopen("opt_mutex.txt", "r");
    if (!fp) {
        fp = fopen("orig.txt", "r");
        if (!fp) {
            printf("ERROR opening input file opt_mutex.txt\n");
            exit(0);
        }
    }
    double opt_mutex_sum_a = 0.0, opt_mutex_sum_f = 0.0, opt_mutex_a, opt_mutex_f;
    for (int i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
        ...
        fscanf(fp, "%s %s %lf %lf\n", append, find,&opt_mutex_a, &opt_mutex_f);
        opt_mutex_sum_a += opt_mutex_a;
        opt_mutex_sum_f += opt_mutex_f;
    }
    fprintf(output, "append() %lf %lf %lf\n", orig_sum_a / 100.0,
            opt_sum_a / 100.0, opt_mutex_sum_a / 100.0);
    fprintf(output, "findName() %lf %lf %lf", orig_sum_f / 100.0,
            opt_sum_f / 100.0, opt_mutex_sum_f / 100.0);
    fclose(fp);
    fclose(output);
